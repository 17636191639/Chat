...
void ollist_init(ollist *list)
{
    ...
    list->head->m_sock = NULL;
    //list->head->id = 0;
    list->head->next = NULL;
}
void insert_ollist(ollist *list, TcpSocket *socket, char *id)
{
    struct loginfo *p = mk_loginfo(socket, id);
...
struct loginfo *mk_loginfo(TcpSocket *socket, char *id)
{
