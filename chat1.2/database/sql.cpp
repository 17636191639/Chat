...
int insert_userinfo(struct CSQL *p, const char *id, const char *pswd, const char *name)
{
    //字符串拼接
    const char *sql = "insert into user values('%s', '%s', '%s');";
    char sql_buf[100] = {0};

    sprintf(sql_buf, sql, id, pswd, name);
    printf("sprintf: %s\n", sql_buf);
    /////////////////////////////////////////////////////
    int ret = mysql_query(p->conn_ptr, sql_buf); //执行SQL语句  执行成功返回0
    if(ret)
    {
       return 0;
    }else
